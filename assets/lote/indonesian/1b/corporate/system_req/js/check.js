...
// START editing after this line ****************
		...
		// --- Plugins stuff
		var nFLVersReq	= 8.0;	// Required Adobe Flash Player plugin version; [float|false] (float = 1.3 etc.)
			...
			if (navigator.plugins && navigator.plugins.length && objFLPlugin){
				if (objFLPlugin){	
					if (objFLPlugin.description){
						...
						nVersionIs = sFLPluginDesc.substring(sFLPluginDesc.indexOf('sh ')+3,sFLPluginDesc.indexOf('.'));
					}
				}
				else {
