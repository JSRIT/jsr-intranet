...
$(document).ready(function(){
	...
	// show hide double toggle (double)
	$('.sh_toggle1').click(function(){
		
		if ($(this).attr('class') !== 'sh_toggle1 ch'){ // exception for "click here links"
			$(this).toggleClass('down');	
		}
		$('.' + $(this).attr('id') + '_c1').slideToggle('slow');
		return false;		
	...
	// true false toggle (for select list interactions; with default)
	$('.tfsd select').change(function(){				  
	...
	*/
	...
	// quiz MCSS
	/* 
	<div class="quiz_mcss" id="q1_q">
		<ol>
			<li><question_text>
				<ol type="a">
					<li><input type="radio" name="q2_1" value="0" />&nbsp;<choice_text></li>
					<li><input type="radio" name="q2_1" value="1" />&nbsp;<choice_text></li>
				</ol><br />
				<span class="hidden ans" id="q2_1_ans">Correct. (<correct_answer>)<br /></span>
				<span class="hidden fls" id="q2_1_fls">Incorrect. <primary_feedback>.<br /></span>
				<span class="hidden fls" id="q2_1_fls2">Incorrect. <secondary_feedback>.<br /></span>
				... etc
			</li>
		</ol>
		<input type="hidden" id="q1_att" value="1" />
		<input type="hidden" id="q1_maxatt" value="2" />
		<input type="hidden" id="q1_pergroup" value="2" />
		<input type="hidden" id="q1_fls2" value="<generic_final_comment" />
		<input type="button" class="btn_answers" id="q1" value="check answers" />
	</div>
	*/
	...
	// true false toggle (for select list interactions)
	$('.slctquiz .btn_answers').click(function(){				  
		
		var counter = 0;
		
		$('.slctquiz select').each(function(){
			
			if ($(this).val() == 1){
				$(this).siblings('span.fls').hide();
				$(this).siblings('span.ans').show();
				counter++;
			}
			else {
				$(this).siblings('span.ans').hide();
				$(this).siblings('span.fls').show();
				counter++;	
			}
		});
		
		if (counter == 0){
			alert('You have to select a response for each option.');	
		}
		
	});
	
	// true false toggle (for select list interactions + specific feedback)
	$('.slctquiz_sf .btn_answers').click(function(){				  
		
		var counter = 0;
		
		$('.slctquiz_sf select').each(function(){
			...
			if ($(this).val() == 1){
				...
				var thisID = $(this).children('option:selected')[0].id;
				if (thisID !== ''){
					$('#' + thisID + '_f').show();	
				}
				else {
	...
	$('.quiz_mcss .btn_answers').click(function(){
						
		var nAttempts = $('#' + $(this).attr('id') + '_att').val();
		var nMaxAttempts = $('#' + $(this).attr('id') + '_maxatt').val();
		var nPerGroup = $('#' + $(this).attr('id') + '_pergroup').val();
		
		var bAllFilledOut = false;
		
		var sFalse2 = '#' + $(this).attr('id') + '_fls2';
		
		var nTotalAnswers = 0;
		var nTotalAnswered = 0;
		
		$('#' + $(this).attr('id') + '_q input:radio').each(function(){
			nTotalAnswers++; // total amount of radiobuttons in quiz
		});
		$('#' + $(this).attr('id') + '_q input[@type=radio]:checked').each(function(){
			nTotalAnswered++; // total amount of radiobuttons checked														 
		});
	
		bAllFilledOut = ((nTotalAnswers/nPerGroup) == nTotalAnswered)? true : false;
		...
		if (bAllFilledOut == true){ // all questions have been attempted, show feedback
			...
			$('#' + $(this).attr('id') + '_q input[@type=radio]:checked').each(function(){
				...
				var thisFalse = '#' + thisName + '_fls';
				var thisAnswer = '#' + thisName + '_ans';
				var thisVal = $(this).val();
	
				if (thisVal == 1){ // if the right answer has been given or is part of the possible alt answers accepted
				...
				else if ((thisVal !== 1) && (nAttempts > 1)){ // on second attempt feedback changes (either all same or individual)
